...
public class StudentRestAdapater {
    ...
    @PostMapping("/v1/api")
    public ResponseEntity<StudentResponse> save(@RequestBody StudentCreateRequest studentRequest) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(mapper.studentResponse(servicePort.save(mapper.toStudent(studentRequest))));
    }

    @PutMapping("/v1/api/{id}")
    public StudentResponse update(@PathVariable Long id, @RequestBody StudentCreateRequest studentRequest) {
